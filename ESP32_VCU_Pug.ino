...
byte0 goes 00->01 as soon as 0x285h is received.
byte1 seems to be current report (dec)/10
byte3 seems to be inlet temperature (dec-40)
byte4 seems to be outlet temperature (dec-40)
byte7 h70 when HV is <175Vdc, h00 when HV >175Vdc. So 175V seems to be the low-voltage limit.
...
// Input pins
int PP_pin = 35; // PP input connected to pin 35
int BMS_pin = 32; // input connected to pin 33
int Heater_pin = 33; // heater input connected to pin 33
int AC_pin = 14; // AC input connected to pin 14
int Enable_pin = 39; // +12V Enable input connected to pin 39
int CP_pin = 23; // CP input pin to sense PWM
int Auxsense_pin = A0; //analog sensing of 12V battery 

// Digital Output pins
int DCDC_active = 26; // relay output connected to pin 4
int CP_relay = 19; // relay output connected to pin 19
int Disable_pin = 2; // Output for Disable pump relay 2 
int Buzzer_pin = 13; // Buzzer pin 13

//PWM output pins
int PWM3_pin = 22;
int PWM2_pin = 5;
int PWM1_pin = 25; //fuel gage output
...
const int PWM3Resolution = 10;
const int PWM3_MAX_DUTY_CYCLE = (int)(pow(2, PWM3Resolution) - 1);

const int L = 242; // lowest permissible  
const int H = 328; // highest permissible
...
bool chargerV1 = false; //charger  threshold is not reached
bool chargerV2 = false; //charger  threshold is not reached
bool auxState = false; //create + name flag for aux 
...
Serial.println("Ready ...!");  
  ...
  timerAlarmWrite(timer, 10000, true);      //Initialize the timer
...
pinMode(Enable_pin,INPUT); // 12V input pin to sense ignition
...
//pinMode(Throttle1_pin,INPUT); // Sense throttle signal
//pinMode(Throttle2_pin,INPUT); // Sense throttle signal
//pinMode(PWM1_pin, OUTPUT);
//pinMode(PWM2_pin, OUTPUT);
//pinMode(PWM3_pin, OUTPUT);
pinMode(Pump_pin, OUTPUT);
pinMode(HVDC_pin, OUTPUT);
...
void loop()
{ 
  // resetting WDT every 3s, programmed to run indefinitely or untill stuck
  if (millis() - lasti >= 2900) {
...
Auxsense_val = analogRead(Auxsense_pin);
Serial.println(Auxsense_val);

if(digitalRead(Heater_pin) == LOW) { // if heater pin is ON
Serial.println("Heater ON");  
Heater_flag = true;
digitalWrite(Heat_pin,HIGH); // turn on heater pump
}

else { // if heater pin is OFF 
Heater_flag = false;
digitalWrite(Heat_pin,LOW); // turn off heater pump
}
...
if(digitalRead(Enable_pin) == HIGH) { // if Enable pin is ON
...
else { // if Enable pin is OFF 
Enable_flag = false;
Serial.println("Car is off"); 
}

if (Auxsense_val < 2930) { // if aux voltage is low and DCDC is off
Serial.println("Aux voltage too low");  
auxState = true; // set the flag to true   
elapsedtime=millis();  
}

if (auxState == true) { // if aux voltage is low and DCDC is off
Serial.println("Auxstate");  
}

DCDCauxcharge(); // setup to charge aux battery via can report
Enable(); //function to run DCDC with coolant pump from multiple sources 
...
void sendCANframeDCDC() { // Tesla DCDC 14V signal 0x3D8 MSG DD 06 00
 CAN_FRAME txFrame;
    txFrame.rtr = 0;
    txFrame.id = 0x3D8; 
...
void sendCANframeB() { // Outlander charger
 CAN_FRAME txFrame;
    ...
    else if(chargerV2 == true) { // if Charger senses more than 322V
...
//void fuel_gage() { //simple function to calculate remaining energy
//dutyCycle = voltage*2;
//dutyCycle = map(dutyCycle, voltageL, voltageH, 0, PWM1_MAX_DUTY_CYCLE); //map(value, fromLow, fromHigh, toLow, toHigh);
//ledcWrite(PWM1Channel, dutyCycle);
//}

//void volts() { //Protection on two sides of SOC if voltage is at the edge disable car and stop charging
//if (voltage < 144) { 
//digitalWrite(Disable_pin,HIGH); // turn on relay to disable driving
//}}
...
void DCDCauxcharge()
{ 
if ((auxState == true) && (digitalRead(Enable_pin) == LOW)) { // auxvoltage went below 12.5V and we are not driving
...
void Enable() //Control of DCDC and coolant pump
{
if ((PP_flag == true) || (Enable_flag == true)) // If EVSE connected or car is started
{
digitalWrite(Pump_pin,HIGH); // turn on Pump relay 
digitalWrite(DCDC_active,HIGH); // turn on DCDC_active relay 
} 
//else {
//digitalWrite(Pump_pin,LOW); // turn on Pump relay    
//digitalWrite(DCDC_active,LOW); // turn off DCDC_active relay
//}
}
